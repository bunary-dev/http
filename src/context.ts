 ...
 *
 * Centralises context construction so that the body readers (`ctx.body.*`),
 * the cookie jar (`ctx.cookies`), and the response helpers (`ctx.json`,
 * `ctx.text`, `ctx.html`, `ctx.redirect`, `ctx.status`) are available
